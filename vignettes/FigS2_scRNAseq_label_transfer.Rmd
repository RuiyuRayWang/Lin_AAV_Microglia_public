...
We use `Seurat` to jointly analyze the query dataset (Smart-seq2) and reference dataset (10X). Specifically, the reference dataset was created with controlled experiments of three treatment groups: Saline, 10mg/kg Poly(i:c) and 0.83mg/kg LPS (administered i.p.). This provides us with a high quality "mini-atlas" that characterizes the transcriptome of microglia under healthy (Saline) and immune-active (Poly(i:c) and LPS) states.

By leveraging the "microglia state" information contained in the reference dataset, we use the `Seurat` anchor method to transfer the "state" labels from the reference dataset to the query dataset. In doing so, we are able to make predictions about the state of AAV-transduced/untransduced microglia in the query, as shown below.
