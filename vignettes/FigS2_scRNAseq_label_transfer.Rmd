...
The input files of this vignette are processed `h5Seurat` objects, produced by the other two vignettes (`FigS2_scRNAseq_query_QC.Rmd`, `FigS2_scRNAseq_reference_QC.Rmd`). The output is a Seurat object merging both query and reference data in the `.h5Seurat` format.
