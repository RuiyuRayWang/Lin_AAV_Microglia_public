...
```{r}
f2a1 <- DimPlot(querydata, pt.size = 2, group.by = "group", cols = brewer.pal(n = 12, name = "Paired")[c(7,6)]) +
  ggtitle(NULL) +
  theme_minimal() +
  theme(
...
as.data.frame(Embeddings(refquery[["umap"]])) %>%
  ...
  scale_color_manual(values = brewer.pal(n = 12, name = "Paired")[c(5,6,1,3,8,2)]) +
