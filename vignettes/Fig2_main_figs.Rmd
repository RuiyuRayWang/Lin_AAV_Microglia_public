...
subtitle: "Label Transfer (Figure 2)"
...
knit: (function(input_file, encoding) {
    ...
    rmarkdown::render(input_file,
      encoding=encoding,
      output_file=file.path(dirname(input_file), out_dir, 'Fig2_main_figs.html'))})
---
...
```{r}
f2a1 <- DimPlot(querydata, pt.size = 3, group.by = "group", cols = brewer.pal(n = 12, name = "Paired")[c(7,6)]) +
  ggtitle(NULL) +
  theme_minimal() +
  theme(
...
```{r}
f2a2 <- DimPlot(refdata, group.by = "treat", pt.size = 2, shuffle = TRUE) +
...
In the publication, this plot is rotated 180 degrees for better visualization.
...
as.data.frame(Embeddings(refquery[["umap"]])) %>%
  ...
  scale_color_manual(values = brewer.pal(n = 12, name = "Paired")[c(5,6,1,3,2,8)]) +
