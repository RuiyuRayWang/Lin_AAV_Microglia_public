...
This vignette reproduces analyses related to scRNAseq in the original publication (Lin et al., 2022).  
...
We found that the single cell transcriptome acquired by 10X Chromium (3' Chemistry v3) captured transcripts of the AAV-packaged transgene (mScarlet) at a very low rate (data not published). Therefore we carried out deep single cell sequencing using a modified protocol based on a Smart-seq2/STRTseq hybrid protocol (Zhang et al., 2020)[^2] on *in vivo* transduced microglia. The Smart-seq2 based method captured mScarlet transcripts with substantially higher efficiently.
...
```{r}
data <- Read10X_h5(filename = '../data/GSE197743/AAV_MG_filtered_feature_bc_matrix.h5')
```

Create Seurat object.
```{r}
refdata <- CreateSeuratObject(data, project = "MG_REF", names.field = 2, names.delim = "-")
```

Update metadata.
```{r}
Idents(refdata) <- "orig.ident"
refdata <- RenameIdents(refdata, `1` = "MG_S", `2` = "MG_P", `3` = "MG_L")
refdata[["group"]] <- Idents(refdata)

Idents(refdata) <- "orig.ident"
refdata <- RenameIdents(refdata, `1` = "Saline", `2` = "Poly(i:c)", `3` = "LPS")
refdata[["treat"]] <- Idents(refdata)

refdata[["id"]] <- "Reference"
refdata[["tech"]] <- "10x"
```
